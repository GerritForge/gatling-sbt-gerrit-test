  ...
  val makeChangeWipScn: ScenarioBuilder =
    setupAuthenticatedSession("Make change WIP")
      ...
      .exec(
        authenticatedPostRequest(
          "Mark Change as WIP",
          s"/changes/${testConfig.project}~#{newChangeNumber}/wip"
        )
      )
      .exec(
        authenticatedPostRequest(
          "Mark Change as Ready",
          s"/changes/${testConfig.project}~#{newChangeNumber}/ready"
        )
      )
        ...
        http("get change details")
          ...
          .resources(
            ...
            http("get cherry picks")
              .get(
                s"/changes/?O=a&q=project%3A${testConfig.project}%20change%3A#{changeId}%20-change%3A#{changeNum}%20-is%3Aabandoned"
              ),
