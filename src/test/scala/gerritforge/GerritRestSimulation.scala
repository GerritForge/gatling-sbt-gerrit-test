...
class GerritRestSimulation extends Simulation {
  ...
  val anonymousUserChangeList = scenario("Anonymous user").exec(listAndCommentChanges())
  val authenticatedChangeList = scenario("Regular user").feed(randomReview).exec(listAndCommentChanges(testConfig.accountCookie, testConfig.xsrfToken))
