...
object GerritRealLifeSimulation {
  ...
  private val scenariosPct = Map(
    httpCloneScenario   -> 0.82,
    postCommentScenario -> 0.11,
    receivePackScenario -> 0.04,
    submitScenario      -> 0.02,
    abandonScenario     -> 0.01
  )

  private val httpCloneDuration =
    (testConfig.duration * scenariosPct(httpCloneScenario)).toSeconds seconds
  private val postCommentDuration =
    (testConfig.duration * scenariosPct(postCommentScenario)).toSeconds seconds
  private val receivePackDuration =
    (testConfig.duration * scenariosPct(receivePackScenario)).toSeconds seconds
  private val submitDuration =
    (testConfig.duration * scenariosPct(submitScenario)).toSeconds seconds
  private val abandonDuration =
    (testConfig.duration * scenariosPct(abandonScenario)).toSeconds seconds
