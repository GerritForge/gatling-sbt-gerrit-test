...
object SuccessfulLogin extends OauthScenarioBase {
  ...
  override def scnActions: ChainBuilder =
    exec(browseToGerritLogin)
      ...
      .exec { session =>
        val redirectToGerritUrl = session("redirectToGerritUrl").as[String]
        session.set(
          "gerritOauthPluginRedirect",
          redirectToGerritUrl.replace(oauthConfig.gerritRedirectUrl, simulationConfig.httpUrl.get)
        )
      }
      .exec(
        http("A valid user is redirected to Gerrit and is logged in")
          .get("#{gerritOauthPluginRedirect}")
          .check(
            status.is(302),
