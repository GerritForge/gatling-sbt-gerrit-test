...
Universal / mappings ++= {
  ...
  val sshFiles = Seq("id_rsa", "id_rsa.pub", "config").map { fileName =>
    new File(fileName) -> s"/staging/.ssh/$fileName" //TODO: These are being written in `/opt/gatling/root/.ssh`, rather than `/root/.ssh`
  }
  val gitConfig = Seq(".gitconfig").map { fileName =>
    new File(fileName) -> s"/staging/$fileName" //TODO: same todo as above
  }
...
dockerCommands ++= {
  ...
  val sshFiles = Seq("id_rsa", "id_rsa.pub", "config").map { fileName =>
    Cmd("Add", s"/opt/gatling/staging/.ssh/$fileName", s"/root/.ssh/$fileName") //TODO: These are being written in `/opt/gatling/root/.ssh`, rather than `/root/.ssh`
  }
  val gitConfig = Seq(".gitconfig").map { fileName =>
    Cmd("Add", s"/opt/gatling/staging/$fileName", s"/root/$fileName") //TODO: same todo as above
  }
  sshFiles ++ gitConfig
}
